...
import './index.css'
import CreateTab from '../TabItem/index'
import CreateApp from '../AppItem/index'
...
class AppStore extends Component {
  ...
  filteredTabItems = () => {
    ...
    const filteredApps = appsList.filter(each => each.category === tabId)
  ...
  render() {
    const finalApps = this.filteredTabItems()
    return (
      <div className="bg">
        ...
        <ul className="apps-ul">
          {finalApps.map(eachApp => (
            <CreateApp details={eachApp} key={eachApp.appId} />
          ))}
        </ul>
      </div>
    )
